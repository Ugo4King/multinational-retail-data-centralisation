...
class DataCleaning:
    ...
    def clean_user_data(self):
        ...
        self.df['phone_number'] = self.df.apply(lambda x: x['phone_number'][-10] if len(x['phone_number'])>10 else x['phone_number'], axis=1)
        ...
        self.df['join_date'] = self.df['join_date'].apply(date_format)
    ...
    def clean_products_data(df):
        ...
        self.df['product_price'] = self.df['product_price'].str.strip('£')
        ...
        self.df['date_added'] = pd.to_datetime(df['date_added'], format='%Y-%m-%d')
